...
def test_file_swapping(library: libraries_models.Library) -> None:
    ...
    foo_record = GenericHandler.objects.get(content=b"foo")
    bar_record = GenericHandler.objects.get(content=b"bar")
    ...
    assert foo_record.files.filter(path="two").exists()
    assert bar_record.files.filter(availability__isnull=False).count() == 1
    assert bar_record.files.filter(path="one").exists()
...
def test_complicated_swapping(library: libraries_models.Library) -> None:
    ...
    first_record = GenericHandler.objects.get(content=b"content1")
    baz_file = first_record.files.get(availability__isnull=False)
    assert baz_file.path == "baz"

    second_record = GenericHandler.objects.get(content=b"content2")
